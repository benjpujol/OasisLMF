...
from ..utils.concurrency import multithread
...
def load_gul_items(self, canexp_profile, canexp_fp, keys_fp):
    """
    Packages GUL items generated by ``generate_gul_items`` into a Pandas frame.
    ...
    try:
        ...
        gul_items_df = pd.DataFrame(data=generate_gul_items(cep, canexp_df, keys_df), dtype=object)
...
def write_items_file(self, gul_items_df, items_fp):
    ...
    """
    fp = as_path(''.join(items_fp), preexists=False)
    try:
        gul_items_df.to_csv(
...
def write_coverages_file(self, gul_items_df, coverages_fp):
    ...
    """
    fp = as_path(''.join(coverages_fp), preexists=False)
    try:
        gul_items_df.to_csv(
...
def write_gulsummaryxref_file(self, gul_items_df, gulsummaryxref_fp):
    ...
    """
    fp = as_path(''.join(gulsummaryxref_fp), preexists=False)
    try:
        gul_items_df.to_csv(
...
def write_gul_files(self, canexp_profile, canexp_fp, keys_fp, items_fp=None, coverages_fp=None, gulsummaryxref_fp=None):
    """
    Writes the standard Oasis GUL files, namely::
        ...
        gulsummaryxref.csv
    """
    if not (items_fp or coverages_fp or gulsummaryxref_fp):
        raise OasisException('No items, coverages or GUL summaryxref file paths provided - please provide at least one')
