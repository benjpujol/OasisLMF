...
def get_coverage_level_il_terms(level_profile, level_agg_profile, level_items, canexp_df, canacc_df):

    lid = level_items[0]['level_id']
    ...
    lap = level_agg_profile

    agg_key = tuple(v['field'].lower() for v in six.itervalues(lap['FMAggKey']))

    li = sorted([it for it in six.itervalues(level_items)], key=lambda it: tuple(it[k] for k in agg_key))
...
def get_layer_level_il_terms(level_profile, level_agg_profile, level_items, canexp_df, canacc_df):
...
def get_sub_layer_non_coverage_level_il_terms(level_profile, level_agg_profile, level_items, canexp_df, canacc_df):
...
def get_level_il_terms(level_profile, level_agg_profile, level_items, canexp_df, canacc_df):

    level_id = level_items[0]['level_id']

    if level_id == OASIS_FM_LEVELS['coverage']['id']:
        return [it for it in get_coverage_level_il_terms(level_profile, level_agg_profile, level_items, canexp_df, canacc_df)]
    elif level_id in range(OASIS_FM_LEVELS['combined']['id'], OASIS_FM_LEVELS['layer']['id']):
        return [it for it in get_sub_layer_non_coverage_level_il_terms(level_profile, level_agg_profile, level_items, canexp_df, canacc_df)]
    elif level_id == OASIS_FM_LEVELS['layer']['id']:
        return [it for it in get_layer_level_il_terms(level_profile, level_agg_profile, level_items, canexp_df, canacc_df)]


def get_policytc_ids(il_items_df):

    columns = [
        col for col in il_items_df.columns if not col in ('limit', 'deductible', 'deductible_min', 'deductible_max', 'attachment', 'share', 'calcrule_id',)
    ]

    policytc_df = il_items_df.drop(columns, axis=1).drop_duplicates()
